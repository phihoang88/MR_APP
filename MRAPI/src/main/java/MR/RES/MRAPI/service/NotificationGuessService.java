...
@Slf4j
@Service
public class NotificationGuessService {

    @Value("${app.firebase-config-guess}")
    ...
    private void initialize() {
        try {
            ...
            if (FirebaseApp.getApps().size() == 1) {
                this.firebaseApp = FirebaseApp.initializeApp(options,"guess");
            } else {
                this.firebaseApp = FirebaseApp.getInstance("guess");
            }
        } catch (IOException e) {
